...
*/
static LocVar *localua_debuginfo (FuncState *fs, int vidx) {
...
static void removevars (FuncState *fs, int tolevel) {
  ...
  while (fs->nactvar > tolevel) {
    LocVar *var = localua_debuginfo(fs, --fs->nactvar);
...
static void lastlistfield (FuncState *fs, ConsControl *cc) {
  ...
  if (hasmultret(cc->v.k)) {
    ...
    luaK_setlist(fs, cc->t->u.info, cc->na, NC_LUA_MULTRET);
...
static void funcargs (LexState *ls, expdesc *f, int line) {
  ...
  if (hasmultret(args.k))
    nparams = NC_LUA_MULTRET;  /* open call */
  else {
    if (args.k != VVOID)
...
static void localfunc (LexState *ls) {
  ...
  localua_debuginfo(fs, fvar)->startpc = fs->pc;
}
...
static void retstat (LexState *ls) {
  ...
  if (block_follow(ls, 1) || ls->t.token == ';')
    ...
    if (hasmultret(e.k)) {
      ...
      nret = NC_LUA_MULTRET;  /* return all values */
    }
    else {
...
LClosure *luaY_parser (nc_lua_flow_t *L, ZIO *z, Mbuffer *buff,
