...
#include <string.h>
// //
const char lua_ident[] = "$LuaVersion: " NC_LUA_COPYRIGHT " $" "$LuaAuthors: " NC_LUA_AUTHORS " $";
...
#define ispseudo(i)		((i) <= NC_LUA_REGISTRYINDEX)
// test for upvalue
#define isupvalue(i)		((i) < NC_LUA_REGISTRYINDEX)

static TValue* index2value(nc_lua_flow_t* L, int idx) {
    CallInfo* ci = L->ci;
    if (idx > 0) {
        StkId o = ci->func + idx;
    ...
    else if (idx == NC_LUA_REGISTRYINDEX)
        return &G(L)->l_registry;
    else {  /* upvalues */
        idx = NC_LUA_REGISTRYINDEX - idx;
...
static StkId index2stack(nc_lua_flow_t* L, int idx) {
    CallInfo* ci = L->ci;
    if (idx > 0) {
        StkId o = ci->func + idx;
...
NC_API int lua_checkstack(nc_lua_flow_t* L, int n) {
    ...
    // stack large enough?
    if (L->stack_last - L->top > n) { res = 1; }  // yes; check is OK;
    else {  // no; need to grow stack;
        int inuse = cast_int(L->top - L->stack) + EXTRA_STACK;
        if (inuse > NC_LUAI_MAXSTACK - n) res = 0;     // can grow without overflow? - no;
        else  res = luaD_growstack(L, n, 0);        // try to grow stack;
    }
...
NC_API lua_Number lua_version(nc_lua_flow_t* L) {
    UNUSED(L);
    return NC_LUA_VERSION_NUM;
}

// --==<basic_stack_manipulation>==--
...
NC_API void lua_settop(nc_lua_flow_t* L, int idx) {
    ...
    if (diff < 0 && hastocloseCfunc(ci->nresults))
        luaF_close(L, L->top + diff, NC_LUA_OK);
...
static void reverse(nc_lua_flow_t* L, StkId from, StkId to) {
    for (; from < to; from++, to--) {
        TValue temp;
...
NC_API void lua_copy(nc_lua_flow_t* L, int fromidx, int toidx) {
    ...
    setobj(L, to, fr);
    if (isupvalue(toidx)) luaC_barrier(L, clCvalue(s2v(L->ci->func)), fr); // function upvalue?
    // NC_LUAREGISTRYINDEX does not need gc barrier (collector revisits it before finishing collection);
    lua_unlock(L);
}
...
NC_API int lua_type(nc_lua_flow_t* L, int idx) {
    ...
    return (isvalid(L, o) ? ttype(o) : NC_LUA_TNONE);
}

NC_API const char* lua_typename(nc_lua_flow_t* L, int t) {
    UNUSED(L);
    api_check(L, NC_LUA_TNONE <= t && t < NC_LUA_NUMTYPES, "invalid type");
    return ttypename(t);
}
...
NC_API int lua_rawequal(nc_lua_flow_t* L, int index1, int index2) {
    const TValue* o1 = index2value(L, index1);
    const TValue* o2 = index2value(L, index2);
    return (isvalid(L, o1) && isvalid(L, o2)) ? luaV_rawequalobj(o1, o2) : 0;
}

NC_API void lua_arith(nc_lua_flow_t* L, int op) {
    lua_lock(L);
    if (op != NC_LUA_OPUNM && op != NC_LUA_OPBNOT)
...
NC_API int lua_compare(nc_lua_flow_t* L, int index1, int index2, int op) {
    const TValue* o1;
    const TValue* o2;
    int i = 0;
    ...
    if (isvalid(L, o1) && isvalid(L, o2)) {
        switch (op) {
        case NC_LUA_OPEQ: i = luaV_equalobj(L, o1, o2); break;
        case NC_LUA_OPLT: i = luaV_lessthan(L, o1, o2); break;
        case NC_LUA_OPLE: i = luaV_lessequal(L, o1, o2); break;
...
NC_API lua_Number lua_tonumberx(nc_lua_flow_t* L, int idx, int* pisnum) {
    lua_Number n = 0;
    const TValue* o = index2value(L, idx);
...
NC_API lua_Integer lua_tointegerx(nc_lua_flow_t* L, int idx, int* pisnum) {
    lua_Integer res = 0;
    const TValue* o = index2value(L, idx);
...
NC_API lua_Unsigned lua_rawlen(nc_lua_flow_t* L, int idx) {
    const TValue* o = index2value(L, idx);
    switch (ttypetag(o)) {
    case NC_LUA_VSHRSTR: return tsvalue(o)->shrlen;
    case NC_LUA_VLNGSTR: return tsvalue(o)->u.lnglen;
    case NC_LUA_VUSERDATA: return uvalue(o)->len;
    case NC_LUA_VTABLE: return luaH_getn(hvalue(o));
    default: return 0;
    }
}
...
static void* touserdata(const TValue* o) {
    switch (ttype(o)) {
    case NC_LUA_TUSERDATA: return getudatamem(uvalue(o));
    case NC_LUA_TLIGHTUSERDATA: return pvalue(o);
    default: return NULL;
    }
}
...
// function to a 'void*', so the conversion here goes through a 'size_t';
// (as the returned pointer is only informative, this conversion should not be a problem);
NC_API const void* lua_topointer(nc_lua_flow_t* L, int idx) {
    const TValue* o = index2value(L, idx);
    switch (ttypetag(o)) {
    case NC_LUA_VLCF: return cast_voidp(cast_sizet(fvalue(o)));
    case NC_LUA_VUSERDATA: case NC_LUA_VLIGHTUSERDATA:
        return touserdata(o);
    default: {
        if (iscollectable(o))
            return gcvalue(o);
...
// pushes on the stack a string with given length;
// avoid using 's' when 'len' == 0 (as 's' can be NULL in that case),
// due to later use of 'memcmp' and 'memcpy';
...
NC_API const char* lua_pushvfstring(nc_lua_flow_t* L, const char* fmt,
    va_list argp) {
    const char* ret;
    lua_lock(L);
...
static int auxgetstr (nc_lua_flow_t *L, const TValue *t, const char *k) {
  const TValue *slot;
...
NC_API int lua_getglobal (nc_lua_flow_t *L, const char *name) {
  ...
  return auxgetstr(L, luaH_getint(reg, NC_LUA_RIDX_GLOBALS), name);
}
...
static int finishrawget (nc_lua_flow_t *L, const TValue *val) {
...
static Table *gettable (nc_lua_flow_t *L, int idx) {
...
NC_API void lua_createtable (nc_lua_flow_t *L, int narray, int nrec) {
  Table *t;
  lua_lock(L);
  t = luaH_new(L);
...
*/
static void auxsetstr (nc_lua_flow_t *L, const TValue *t, const char *k) {
  const TValue *slot;
  TString *str = luaS_new(L, k);
  api_checknelems(L, 1);
  if (luaV_fastget(L, t, str, slot, luaH_getstr)) {
...
NC_API void lua_setglobal (nc_lua_flow_t *L, const char *name) {
  ...
  reg = hvalue(&G(L)->l_registry);
  auxsetstr(L, luaH_getint(reg, NC_LUA_RIDX_GLOBALS), name);
}
...
static void aux_rawset (nc_lua_flow_t *L, int idx, TValue *key, int n) {
  Table *t;
  TValue *slot;
  lua_lock(L);
  api_checknelems(L, n);
...
#define checkresults(L,na,nr) \
     api_check(L, (nr) == NC_LUA_MULTRET || (L->ci->top - L->top >= (nr) - (na)), \
	"results from function overflow current stack size")


NC_API void lua_callk (nc_lua_flow_t *L, int nargs, int nresults,
  ...
  api_check(L, L->status == NC_LUA_OK, "cannot do calls on non-normal thread");
...
static void f_call (nc_lua_flow_t *L, void *ud) {
...
NC_API int lua_pcallk (nc_lua_flow_t *L, int nargs, int nresults, int errfunc,
                        intptr_t ctx, lua_KFunction k) {
  struct CallS c;
  int status;
  ptrdiff_t func;
  ...
  if (k == NULL || !yieldable(L)) {  /* no continuation or no yieldable? */
    ...
    status = NC_LUA_OK;  /* if it is here, there were no errors */
  }
  adjustresults(L, nresults);
  lua_unlock(L);
  return status;
...
NC_API int lua_load (nc_lua_flow_t *L, lua_Reader reader, void *data,
  ...
  if (status == NC_LUA_OK) {  /* no errors? */
    LClosure *f = clLvalue(s2v(L->top - 1));  /* get newly created function */
    if (f->nupvalues >= 1) {  /* does it have an upvalue? */
      /* get global table from registry */
      Table *reg = hvalue(&G(L)->l_registry);
      const TValue *gt = luaH_getint(reg, NC_LUA_RIDX_GLOBALS);
      /* set global table as 1st upvalue of 'f' (may be NC_LUA_ENV) */
...
NC_API int lua_gc (nc_lua_flow_t *L, int what, ...) {
  ...
  switch (what) {
    ...
    case NC_LUA_GCRESTART: {
      luaE_setdebt(g, 0);
      g->gcrunning = 1;
      break;
    }
    case NC_LUA_GCCOLLECT: {
      luaC_fullgc(L, 0);
      break;
    }
    case NC_LUA_GCCOUNT: {
    ...
    case NC_LUA_GCSETPAUSE: {
      int data = va_arg(argp, int);
      res = getgcparam(g->gcpause);
      setgcparam(g->gcpause, data);
      break;
    }
    case NC_LUA_GCSETSTEPMUL: {
      int data = va_arg(argp, int);
      res = getgcparam(g->gcstepmul);
      setgcparam(g->gcstepmul, data);
      break;
    }
    case NC_LUA_GCISRUNNING: {
      res = g->gcrunning;
      break;
    }
    case NC_LUA_GCGEN: {
      ...
      res = isdecGCmodegen(g) ? NC_LUA_GCGEN : NC_LUA_GCINC;
      if (minormul != 0)
...
void lua_setwarnf (nc_lua_flow_t *L, lua_WarnFunction f, void *ud) {
  lua_lock(L);
  G(L)->ud_warn = ud;
  G(L)->warnf = f;
  lua_unlock(L);
...
void lua_warning (nc_lua_flow_t *L, const char *msg, int tocont) {
  lua_lock(L);
...
NC_API void *lua_newuserdatauv (nc_lua_flow_t *L, size_t size, int nuvalue) {
  Udata *u;
  lua_lock(L);
...
static UpVal **getupvalref (nc_lua_flow_t *L, int fidx, int n, LClosure **pf) {
...
NC_API void lua_upvaluejoin (nc_lua_flow_t *L, int fidx1, int n1,
                                            int fidx2, int n2) {
  LClosure *f1;
