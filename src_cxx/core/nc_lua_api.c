#include "nc_lua_api.h"
// native //
#include "nc_lua_core.h"
#include "nc_lua_prefix.h"
#include "nc_lua_dbg.h"
#include "nc_lua_do.h"
#include "nc_lua_func.h"
#include "nc_lua_gc.h"
#include "nc_lua_mem.h"
#include "nc_lua_obj.h"
#include "nc_lua_stt.h"
#include "nc_lua_str.h"
#include "nc_lua_tab.h"
#include "nc_lua_tagm.h"
#include "nc_lua_undump.h"
#include "nc_lua_vm.h"
// standard //
#include <limits.h>
#include <stdarg.h>
...
NC_LUA_API const char* lua_typename(lua_State* L, int t) {
    UNUSED(L);
...
NC_LUA_API int lua_rawequal(lua_State* L, int index1, int index2) {
...
NC_LUA_API int lua_compare(lua_State* L, int index1, int index2, int op) {
    const TValue* o1;
    const TValue* o2;
    int i = 0;
...
NC_LUA_API lua_Number lua_tonumberx(lua_State* L, int idx, int* pisnum) {
    lua_Number n = 0;
    const TValue* o = index2value(L, idx);
...
NC_LUA_API lua_Integer lua_tointegerx(lua_State* L, int idx, int* pisnum) {
    lua_Integer res = 0;
    const TValue* o = index2value(L, idx);
...
NC_LUA_API const char* lua_pushvfstring(lua_State* L, const char* fmt,
    va_list argp) {
    const char* ret;
    lua_lock(L);
...
NC_LUA_API void lua_createtable (lua_State *L, int narray, int nrec) {
  Table *t;
  lua_lock(L);
  t = luaH_new(L);
...
NC_LUA_API void lua_callk (lua_State *L, int nargs, int nresults,
...
NC_LUA_API int lua_pcallk (lua_State *L, int nargs, int nresults, int errfunc,
                        lua_KContext ctx, lua_KFunction k) {
  struct CallS c;
  int status;
  ptrdiff_t func;
...
NC_LUA_API int lua_load (lua_State *L, lua_Reader reader, void *data,
...
NC_LUA_API void *lua_newuserdatauv (lua_State *L, size_t size, int nuvalue) {
  Udata *u;
  lua_lock(L);
...
NC_LUA_API void lua_upvaluejoin (lua_State *L, int fidx1, int n1,
                                            int fidx2, int n2) {
  LClosure *f1;
