...
namespace NWLUA
{
	...
	// --getters
	template <> v1s lua_engine::get_lua_value_glb<v1s>(cstr name) {
		v1s result = 0;
		if (lua_move_tglb(name, LT_NUM)) {
			result = static_cast<v1s>(lua_tonumberx(m_lstate, -1, 0));
		}
		else { result = 0; }
		lua_clear(1);
		return result;
	}
	template <> v1s lua_engine::get_lua_value_loc<v1s>(cstr name) {
		v1s result = 0;
	...
	template <> v1s lua_engine::get_lua_value<v1s>(cstr strLoc, cstr name) {
		v1s result = 0;
	...
	// --setters
	template <> bool lua_engine::set_lua_value_glb<v1s>(const v1s& tValue, cstr name) {
	...
	template <> bool lua_engine::set_lua_value_loc<v1s>(const v1s& tValue, cstr name) {
	...
	template <> bool lua_engine::set_lua_value<v1s>(const v1s& tValue, cstr strLoc, cstr name) {
	...
	// --==<core_methods>==--
	//nwlua_table_info Luacmp_sys::lrtcmp_sys = lua_table_info("cmp_sys");
	//nwlua_table_info Luaent_sys::lrtent_sys = lua_table_info("ent_sys");
	lua_state* lua_engine::new_lua_state()
	{
		lua_state* lstate = lua_newstate(lua_engine::lua_alloc , &m_MemAllocator);
		//nwlua_state* pLState = luaL_newstate();
		//nwlua_setallocf(pLState, lua_engine::LuaAllocate, &m_MemAllocator);
	...
	bool lua_engine::load_script(const dstr& script)
	{
		lua_reader_state lstate_reader;
		lstate_reader.chunk = &script;
		v1s resultult = 0;
		//nwlua_load(m_lstate, lua_engine::lua_read, &lstate_reader, "lua_chunk", "r");
		
		return resultult == NWLUA_OK;
	}

	bool lua_engine::run_script_str(cstr script)
	{
		bool result = true;
		luaL_loadstr(m_lstate, &script[0]);
		m_info.add_type(lua_type(m_lstate, -1));
		if (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) != NWLUA_OK) {
	...
	bool lua_engine::run_script_file(cstr file_path)
	{
		bool result = true;
		if (luaL_dofile(m_lstate, &file_path[0]) != NWLUA_OK) {
			write_err("could not run the script");
			result = false;
		}
		else { result = true; }
	...
	bool lua_engine::call_lua_func_glb(cstr name)
	{
		bool result = true;
		result = lua_move_tglb(name, LT_FNC);
		if (result) { result = (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) == NWLUA_OK); lua_clear(1); }
		else { write_err("function \"{str}\" was not found;", name); }
		return result;
	}
	bool lua_engine::call_lua_func_loc(cstr name)
	{
		bool bResult = true;
		bResult = lua_move_tloc(name, LT_FNC);
		if (bResult) { bResult = (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) == NWLUA_OK); lua_clear(1); }
		else { write_err("function \"{str}\" was not found;", name); }
		return bResult;
	}
	bool lua_engine::call_lua_func(cstr strLoc, cstr name)
	{
		bool bResult = true;
		bResult = lua_move_t(strLoc, name, LT_FNC);
		if (bResult) { bResult = (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) == NWLUA_OK); lua_clear(); }
		else { write_err("function \"{str}\" was not found;", name); }
		return bResult;
	}
	...
	inline bool lua_engine::lua_move_glb(cstr name)
	{
	...
	inline bool lua_engine::lua_move_loc(cstr name)
	{
	...
	inline bool lua_engine::lua_move(cstr strLoc, cstr name)
	{	// get on the top of the stack some value due to string "tab1.tab2.value"
		dstr str_buf("", 256);		// String buffer for the currently serched name
		stm_io_str stm = stm_io_str(dstr(strLoc) + '.' + name);
	...
	inline bool lua_engine::lua_move_tglb(cstr name, lua_types nLuaType) {
		if (lua_move_glb(name)) { return m_info.get_last_type() == nLuaType; }
		else { return false; }
	}
	inline bool lua_engine::lua_move_tloc(cstr name, lua_types nLuaType) {
		if (lua_move_loc(name)) { return m_info.get_last_type() == nLuaType; }
		else { return false; }
	}
	inline bool lua_engine::lua_move_t(cstr strLoc, cstr name, lua_types nLuaType) {
		if (lua_move(strLoc, name)) { return m_info.get_last_type() == nLuaType; }
		else { return false; }
	}

	inline void lua_engine::lua_clear() {
		v1u lvl_num = lua_gettop(m_lstate);
		lua_clear(lvl_num);
	}
	inline void lua_engine::lua_clear(v1s count)
	{
	...
	inline bool lua_engine::lua_push(lua_func_info& reg_info)
	{
		...
		for (v1u uvi = 0; uvi < up_vals_cpy.size(); uvi++) { // throw all the upvalues on the top of the stack
	...
	inline bool lua_engine::lua_push(lua_table_info& r_reg_tab)
	{
		...
		if (r_reg_tab.meta_table != nullptr) {
			...
			lua_setfield(m_lstate, NWLUA_REGISTRYINDEX, r_reg_tab.meta_table->name);  // registry.name = metatable
	...
	{
		v1s lvl_num = lua_gettop(m_lstate);
		m_info.rmv_type(lvl_num);
		for (v1s ti = 0; ti < lvl_num; ti++) { m_info.add_type(lua_type(m_lstate, -ti)); }
		
		io_sys::get().write_info(
			"lua_stack_info\n"
			"status: {int};\nvalues: {int}\nstack_size: {int};",
			m_lstate->status, m_lstate->stack->val, m_lstate->stack_last
		);
		
		std::cout << m_info;
	}
	inline void lua_engine::write_err(cstr strFormat, ...)
	{
		va_list valArgs;
		va_start(valArgs, strFormat);
		dstr err_commentLog = str_format_var(strFormat, valArgs);
		va_end(valArgs);
		if (lua_isstring(m_lstate, -1)) { err_commentLog += lua_tolstring(m_lstate, -1, 0); }
		lua_clear();
		NWLUA_ERR(&err_commentLog[0]);
	}
	// --==</implementation_methods>==--
	...
	cstr lua_engine::lua_read(lua_state* lstate, ptr reader, size* block_size)
	{
	...
	{
		a_mem_alloc* mem_alloc = static_cast<a_mem_alloc*>(allocator);
		if (block_ptr != nullptr) {
			if (new_size == 0) { mem_alloc->dealloc(block_ptr, old_size); }
			else if (old_size > 0) { block_ptr = mem_alloc->realloc(block_ptr, old_size, new_size); }
		}
		else if (new_size > 0) { block_ptr = mem_alloc->alloc(new_size); }
		return block_ptr;
	}
	// --==</nwlua_delegates>==--
}
...
//inline const lua_func_info* lua_engine::GetRegFunction(cstr name) {
//	RegFunctions::iterator itRegFunc = m_RegFunctions.find(name);
//	return itRegFunc == m_RegFunctions.end() ? nullptr : &itRegFunc->second;
//}
//inline const lua_table_info* lua_engine::GetRegTable(cstr name) {
...
//		lvl_num = lua_move(&reg_info.strLoc[0]);
//		v1s ltype = lua_type(m_lstate, -1);
//		if (ltype == NWLUA_TTABLE) {
//			 Get the function on the top again
//			lua_pushvalue(m_lstate, -2); lvl_num++;
//			 Stack now has ...[-3]function[-2]table[-1]function
//			 Set that function as a part of the table - function goes out
//			lua_setfield(m_lstate, -2, &reg_info.name[0]); lvl_num--;
//			lua_clear(lvl_num);
//		}
//		else { return; }
//	}
...
//}
//void lua_engine::RemoveRegFunction(cstr name) {
//	RegFunctions::iterator itRegFunc = m_RegFunctions.find(name);
//	if (itRegFunc == m_RegFunctions.end()) { return; }
//	m_RegFunctions.erase(itRegFunc);
//}
//void lua_engine::RemoveRegTable(cstr name) {
//	RegTables::iterator itRegTab = m_RegTables.find(name);
//	if (itRegTab == m_RegTables.end()) { return; }
//	m_RegTables.erase(itRegTab);
//}
// -- Predicates
//bool lua_engine::HasTable(cstr name) {
//	ui16 lvl_num = lua_move(name);
//	bool bHasTab = lua_type(m_lstate, -1) == NWLUA_TTABLE;
//	lua_clear(lvl_num);
//	return bHasTab;
//}
//bool lua_engine::HasRegTable(cstr name) { return m_RegTables.find(name) != m_RegTables.end(); }
//bool lua_engine::HasFunction(cstr name) {
//	ui16 lvl_num = lua_move(name);
//	bool bHasFunc = lua_type(m_lstate, -1) == NWLUA_TFUNCTION;
//	lua_clear(lvl_num);
//	return bHasFunc;
//}
//bool lua_engine::HasRegFunction(cstr name) { return m_RegFunctions.find(name) != m_RegFunctions.end(); }
...
//		ui16 lvl_num = lua_move(&r_reg_tab.strLoc[0]);
//		if (lua_type(m_lstate, -1) == NWLUA_TTABLE) {		// The over table is on the stack
