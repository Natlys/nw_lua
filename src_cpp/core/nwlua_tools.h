#ifndef NWLUA_TOOLS_H
#define NWLUA_TOOLS_H
...
namespace NWLUA
{
	...
	public:
		lua_engine_info() { memset(&m_stack_types[0], LT_NON, sizeof(v1s)); }
		// --getters
		inline lua_types get_stack_type(v1u unIndex) { return m_stack_types[unIndex]; }
		inline lua_types get_last_type()              { return m_nlevel == 0 ? LT_NIL : m_stack_types[m_nlevel - 1]; }
		inline v1u get_level()                       { return m_nlevel; }
		// --setters
		void add_type(v1s ltype);
		void rmv_type(v1u count);
		void reset();
		// --operators
